...
function showDedicationText() {
  ...
  const boldLine = 'Para la personita mas especial de mi vida:';
  const restText = `

Lograste que mi año sea diferente, alegras mis dias con tus ocurrencias.

Gracias por acompañarme en cada paso, por entenderme incluso en silencio, y por llenar mis días de amor.

No sé que nos traiga el 2026 pero estoy seguro que quiero que todo sea a tu lado.

Te dedico esta canción.`;

  let i = 0;
  let j = 0;
  let phase = 'bold'; // bold → normal → end

  function type() {
    if (phase === 'bold') {
      // Escribe la línea en negrita
      container.innerHTML =
        `<strong>${boldLine.slice(0, i)}</strong>`;
      i++;

      if (i > boldLine.length) {
        phase = 'normal';
        i = 0;
      }
      ...
      container.innerHTML =
        `<strong>${boldLine}</strong>` +
        restText.slice(0, j).replace(/\n/g, '<br>');
      j++;

      if (j > restText.length) {
        phase = 'end';
      }

      setTimeout(type, restText[j - 2] === '\n' ? 350 : 45);
    }

    else {
      // Al finalizar, convertir la palabra en link
      container.innerHTML =
        `<strong>${boldLine}</strong>` +
        restText
          .replace(/\n/g, '<br>')
          .replace(
            'esta canción',
            '<a href="https://open.spotify.com/intl-es/track/1x57piY4gMV0gFMk1sLb8A?si=ecd5d0ea27274f94" target="_blank">esta canción</a>'
          );
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2025-07-26T00:00:00'); 
